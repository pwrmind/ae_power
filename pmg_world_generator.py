...
# ==========================================
# 2. АРХИТЕКТУРА УГЛУБЛЕННОГО PMG-VAE (LATENT=64)
# ==========================================
...
# ==========================================
# 3. МАТЕМАТИЧЕСКАЯ МОДЕЛЬ ТУРБУЛЕНТНОСТИ (fBm)
# ==========================================

def generate_perlin_noise_2d(size, res):
    """Исправленный генератор с одиночными числовыми шагами для arange."""
    def f(t): return 6*t**5 - 15*t**4 + 10*t**3
    
    # delta — это теперь строго одиночное число float, а не кортеж
    delta = res / size
    
    grid = torch.stack(torch.meshgrid(
        torch.arange(0, res, delta), 
        torch.arange(0, res, delta), 
        indexing='ij'
    ), dim=-1) % 1
...
def generate_fbm_turbulence(size):
    """Смешивает октавы для эффекта реальной природной турбулентности."""
    noise1 = generate_perlin_noise_2d(size, 4)  # Крупные формы
    noise2 = generate_perlin_noise_2d(size, 8)  # Средние завихрения
    noise3 = generate_perlin_noise_2d(size, 16) # Мелкие фрактальные брызги
...
print("Сборка усложненного турбулентного датасета...")
num_images = 512
...
print(f"Турбулентный датасет готов! Форма: {dataset.shape}")
...
# ==========================================
# 4. ТРЕНИРОВКА РАСШИРЕННОЙ МОДЕЛИ (150 ЭПОХ)
# ==========================================
...
print("\n=== СТАРТ СИНТЕЗА ТУРБУЛЕНТНЫХ ПОЛЕЙ ===")
model.train()
...
for epoch in range(epochs):
    ...
    if (epoch + 1) % 15 == 0 or epoch == 0:
        ...
        print(f"Эпоха {epoch+1:03d}/150 | VAE Total Loss: {epoch_loss/num_batches:.2f} | Время: {time.time()-t0:.2f}с")
        print(f"   [PMG Вселенная] Средний параметр Base: {np.mean(all_bases):.4f}")

print("\n=== Пространство вероятностей сформировано! ===")

# ==========================================
# 5. ГЕНЕРАЦИЯ И ФИЗИЧЕСКОЕ СОХРАНЕНИЕ КАРТИНОК
# ==========================================
model.eval()
...
with torch.no_grad():
    print(f"\n=== ГЕНЕРАЦИЯ 16 МИРОВ ИЗ ХАОСА И СОХРАНЕНИЕ В ПАПКУ '{output_dir}' ===")
    
    random_noise_vectors = torch.randn(16, latent_dim).to(device)
    generated_worlds = model.decode(random_noise_vectors)
    
    for idx in range(16):
        img_array = generated_worlds[idx, 0].cpu().numpy()
        img_array = (img_array * 255).astype(np.uint8)
        
        img = Image.fromarray(img_array, mode='L')
        img = img.resize((256, 256), resample=Image.Resampling.NEAREST)
        
        file_path = os.path.join(output_dir, f"world_{idx+1:02d}.png")
        img.save(file_path)
        
    print(f"-> Успешно! 16 уникальных фрактальных текстур сохранены в папку: {os.path.abspath(output_dir)}")
    
    print("\n=== МАТРИЦА ПЕРВОГО СГЕНЕРИРОВАННОГО МИРА (Срез центра 8x8) ===")
    print(np.round(generated_worlds[0, 0, 12:20, 12:20].cpu().numpy(), 2))
